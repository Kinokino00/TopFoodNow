...
import com.example.topfoodnow.infra.role.Role;
import com.example.topfoodnow.infra.role.RoleRepository;
...
public class UserService {
  ...
  private String appBaseUrl;
    ...
    @Override
    @Transactional
    public User addUser(RegisterRequest requestDTO) throws IOException {
        User user = new User();
        ...
        if (requestDTO.getProfilePictureFile() != null && !requestDTO.getProfilePictureFile().isEmpty()) {
            String profilePicUrl = fileStorageService.uploadFile(requestDTO.getProfilePictureFile(), "profile-pictures/");
            user.setProfilePictureUrl(profilePicUrl);
            log.info("註冊用戶 {} 上傳頭像成功。URL: {}", requestDTO.getEmail(), profilePicUrl);
        }

        User savedUser = userRepository.save(user);
        log.info("已為用戶 {} 生成驗證碼並準備發送郵件。", user.getEmail());
        return savedUser;
    }
    ...
    @Override
    @Transactional
    public User updateProfile(Integer userId, UpdateUserProfileRequest updateDTO) throws IOException { // 增加 IOException
        ...
        if (profilePictureFile != null && !profilePictureFile.isEmpty()) {
            // 上傳新圖片
            String newProfilePictureUrl = fileStorageService.uploadFile(profilePictureFile, "profile-pictures/");
            user.setProfilePictureUrl(newProfilePictureUrl);
            log.info("用戶 ID: {} 更新頭像成功。新URL: {}", userId, newProfilePictureUrl);

            // 刪除舊圖片 (如果存在)
            if (oldProfilePictureUrl != null && !oldProfilePictureUrl.isEmpty()) {
                try {
                    fileStorageService.deleteFile(oldProfilePictureUrl);
                    log.info("用戶 ID: {} 成功刪除舊頭像: {}", userId, oldProfilePictureUrl);
                } catch (Exception e) {
                    log.error("用戶 ID: {} 刪除舊頭像失敗: {}. URL: {}", userId, e.getMessage(), oldProfilePictureUrl, e);
                }
            }
            ...
            try {
                ...
                log.info("用戶 ID: {} 成功刪除頭像 (無新頭像上傳): {}", userId, oldProfilePictureUrl);
            } catch (Exception e) {
                log.error("用戶 ID: {} 刪除頭像失敗 (無新頭像上傳): {}. URL: {}", userId, e.getMessage(), oldProfilePictureUrl, e);
    ...
    @Override
    public GetUserProfileResponse getUserProfileForPublicRecommend(Integer userId) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new EntityNotFoundException("未找到用戶 ID: " + userId));

        GetUserProfileResponse dto = new GetUserProfileResponse();
