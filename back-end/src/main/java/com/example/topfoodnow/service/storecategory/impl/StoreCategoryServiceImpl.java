package com.example.topfoodnow.service.storecategory.impl;
...
public class StoreCategoryServiceImpl implements StoreCategoryService {
    ...
    @Override
    @Transactional
    public StoreAndCategoryResponse addStoreCategory(Integer storeId, Integer categoryId) {
        // 檢查店家和分類是否存在
    ...
    @Override
    @Transactional
    public StoreCategoryListResponse addOrUpdateStoreCategories(Integer storeId, List<Integer> categoryIds,
            boolean isAdmin) {
        // 檢查店家是否存在
        Store store = storeRepository.findById(storeId)
        ...
        // 用於收集要返回的分類 DTO
        List<CategoryResponse> addedCategories = new ArrayList<>();
    ...
    @Override
    public StoreCategoryListResponse getSortedCategoriesForStore(Integer storeId) {
        Store store = storeRepository.findById(storeId)
                .orElseThrow(() -> new EntityNotFoundException("Store not found with ID: " + storeId));
        ...
        Set<CategoryResponse> sortedCategories = new LinkedHashSet<>();
        ...
        directAssociations.stream()
                ...
                .collect(Collectors.collectingAndThen(
                        Collectors.toList(),
                        list -> {
                            list.sort(Comparator.comparing(CategoryResponse::getId));
                            return list;
                        }))
        ...
        directAssociations.stream()
                ...
                .collect(Collectors.collectingAndThen(
                        Collectors.toList(),
                        list -> {
                            list.sort(Comparator.comparing(CategoryResponse::getId)); // 按ID排序
                            return list;
                        }))
                .forEach(sortedCategories::add);

        com.example.topfoodnow.controller.store.response.StoreCategoryListResponse responseDTO = new com.example.topfoodnow.controller.store.response.StoreCategoryListResponse();
    ...
    @Override
    public List<StoreAndCategoryResponse> getStoresByCategoryId(Integer categoryId) {
        return storeCategoryRepository.findById_CategoryId(categoryId).stream()
                .map(this::convertToDto)
                .collect(Collectors.toList());
    }

    private StoreAndCategoryResponse convertToDto(StoreCategory model) {
        StoreAndCategoryResponse dto = new StoreAndCategoryResponse();
    ...
    private CategoryResponse convertToCategoryDTO(Category model) {
