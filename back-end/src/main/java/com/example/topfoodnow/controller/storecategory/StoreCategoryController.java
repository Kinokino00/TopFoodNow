package com.example.topfoodnow.controller.storecategory;

import com.example.topfoodnow.controller.storecategory.request.AddOrUpdateStoreCategoriesRequest;
import com.example.topfoodnow.controller.storecategory.request.RemoveStoreCategoryRequest;
...
import com.example.topfoodnow.controller.storecategory.response.AddOrUpdateStoreCategoriesResponse;
import com.example.topfoodnow.service.storecategory.StoreCategoryService;
...
public class StoreCategoryController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "分類添加/更新成功", content = @Content(mediaType = "application/json", schema = @Schema(implementation = AddOrUpdateStoreCategoriesResponse.class))),
    ...
    @PostMapping
    public ResponseEntity<AddOrUpdateStoreCategoriesResponse> addOrUpdateStoreCategories(
            @Valid @RequestBody AddOrUpdateStoreCategoriesRequest request,
            Principal principal) {
        logger.info("管理員嘗試為店家 ID: {} 添加或更新分類。", request.getStoreId());
        try {
            boolean isAdminRequest = isAdmin(principal);
            GetStoreCategoriesResponse updatedStoreCategories = storeCategoryService.addOrUpdateStoreCategories(
                    request.getStoreId(),
                    request.getCategoryIds(),
                    isAdminRequest);
            AddOrUpdateStoreCategoriesResponse response = new AddOrUpdateStoreCategoriesResponse();
            response.setStoreId(updatedStoreCategories.getStoreId());
            response.setStoreName(updatedStoreCategories.getStoreName());
            response.setCategories(updatedStoreCategories.getCategories());
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @DeleteMapping
    public ResponseEntity<Void> removeStoreCategory(
            @Valid @RequestBody RemoveStoreCategoryRequest request) {
        logger.info("管理員嘗試從店家 ID: {} 移除分類 ID: {}。", request.getStoreId(), request.getCategoryId());
        try {
            storeCategoryService.removeStoreCategory(request.getStoreId(), request.getCategoryId());
            return ResponseEntity.noContent().build();
        } catch (jakarta.persistence.EntityNotFoundException e) {
            logger.warn("移除分類失敗：未找到店家 ID: {} 與分類 ID: {} 的關聯。錯誤: {}", request.getStoreId(), request.getCategoryId(), e.getMessage());
            return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
        } catch (Exception e) {
            logger.error("從店家 ID: {} 移除分類 ID: {} 時發生錯誤: {}", request.getStoreId(), request.getCategoryId(), e.getMessage(), e);
