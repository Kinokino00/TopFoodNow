...
public class UserController {
    ...
    public ResponseEntity<AuthResponseDTO> loginProcess(@Valid @RequestBody LoginRequestDTO loginRequest,
        ...
        return ResponseEntity.ok(new AuthResponseDTO(true, "登入成功", jwt, userModel));
    }
    // endregion
...
//    }
    ...
    public ResponseEntity<UserProfileResponseDTO> updateProfile(@Valid @ModelAttribute UserProfileUpdateDTO updateDTO,
                                                                BindingResult bindingResult,
                                                                Principal principal) {
    ...
    public ResponseEntity<String> logout(@RequestBody String token) {
        ...
        try {
            // 從包含 "Bearer " 的標頭中提取 JWT 字串
            String jwt = authorizationHeader.substring(7);

            // 檢查 Token 是否在黑名單中
            if (jwtBlacklistService.isTokenBlacklisted(jwtUtil.extractJti(jwt))) {
                return ResponseEntity.badRequest().body("此 Token 已在黑名單中。");
            }

            // 將 Token 加入黑名單
            jwtBlacklistService.blacklistToken(jwt);
            logger.info("Token 已成功加入黑名單，用戶已登出");

            return ResponseEntity.ok("登出成功");
        } catch (Exception e) {
            logger.error("登出失敗：無效的 Token 或處理失敗", e);
