...
import com.example.topfoodnow.controller.store.request.StoreCategoryRequest;
...
public class StoreCategoryController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "分類添加/更新成功", content = @Content(mediaType = "application/json", schema = @Schema(implementation = StoreCategoryListResponse.class))),
    ...
    @PostMapping
    public ResponseEntity<StoreCategoryListResponse> addOrUpdateCategoriesToStore(
            @Valid @RequestBody StoreCategoryRequest request,
            Principal principal) {
        logger.info("管理員嘗試為店家 ID: {} 添加或更新分類。", request.getStoreId());
        try {
            boolean isAdminRequest = isAdmin(principal);
            StoreCategoryListResponse updatedStoreCategories = storeCategoryService.addOrUpdateStoreCategories(
                    request.getStoreId(),
